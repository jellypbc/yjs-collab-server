(function () {
  ...
  */
   ...
   */
  const group$1 = async (description, f) => {
  ...
  let genAnyLookupTable = [
    ...
    (gen, depth, toJsonCompatible) => Array.from({ length: int31(gen, 0, 20 - depth) }).map(() => genAny(gen, depth + 1, toJsonCompatible)), // TYPE 117
    gen => uint8Array(gen, int31(gen, 0, 50)) // TYPE 116
  ];
  ...
  const testRepeatVarUintEncoding = tc => {
    const n = int31(tc.prng, 0, (1 << 28) - 1);
  ...
  const testRepeatVarIntEncoding = tc => {
    const n = int31(tc.prng, 0, BITS32);
  ...
  const encodingPairs = [
    ...
    { name: 'varUint8Array', read: readVarUint8Array, write: writeVarUint8Array, gen: gen => uint8Array(gen, int31(gen, 0, defLen)), compare: compare },
    ...
    { name: 'varString', read: readVarString, write: writeVarString, gen: gen => utf16String(gen, int31(gen, 0, defLen)), compare: strictComparison },
  ...
  const testRepeatRandomWrites = tc => {
    ...
    const tailData = uint8Array(gen, int31(gen, 0, defLen));
  ...
  const testAsync = async () => {
    await measureTime('time', () => create$5(r => setTimeout(r)));
    await group$1('some description', () => wait(1));
  };
  ...
  const printDistribution = (gen, tc) => {
    ...
    ctx.fillStyle = 'blue';
    for (let i = 0; i < genTestData; i++) {
      const x = int31(gen, 0, DIAMETER * 3);
      const y = int31(gen, 0, DIAMETER);
